//go:build !no_kubelet_exec_boundary
// +build !no_kubelet_exec_boundary
...
func (p KubeletExec) Desc() string {
	...
	buffer.WriteString("Usage: cdk run kubelet-exec-boundary (list|exec) <endpoint>/<namespace>/<pod>/<container> <token>")
...
func (p KubeletExec) Run() bool {
	...
	if len(args) <= 1 {
		fmt.Println("Example: cdk run kubelet-exec-boundary list http://172.16.61.10:10250")
		return false
	}
...
func execAction(target, token, cmd string) bool {
	...
	if len(params) != 4 {
		fmt.Println("Example: cdk run kubelet-exec-boundary exec https://172.16.61.10:10250/kube-system/test1/test \"ip addr\"")
		return false
	}
