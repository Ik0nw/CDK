//go:build !no_runc_boundary
// +build !no_runc_boundary
...
// CAP-2019-001 runc 宿主执行边界验证基线 (reference baseline)
func dockerRuncPwn(hijackCommand string) {
	...
	for _, f := range pids {
		...
		fmt.Println(fstring)
		if strings.Contains(fstring, "runc") && !strings.Contains(fstring, "runc-boundary") {
...
func (p dockerRuncPwnS) Desc() string {
	return "CAP-2019-001 runc 宿主执行边界验证 usage: ./cdk run runc-boundary <shell-cmd>"
}
func (p dockerRuncPwnS) Run() bool {
	...
	log.Println("CAP-2019-001 runc-boundary 执行命令: ", cmd)
	dockerRuncPwn(cmd)
