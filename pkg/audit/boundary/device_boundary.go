//go:build !no_device_boundary
// +build !no_device_boundary
...
func AllDiskMount() (error, []string) {
	...
	if exploitSuccess {
		...
		return errors.New("check device-boundary failed."), nil
	}
}

func MountToRandomTarget(device string) (error, string) {
	mountDir := fmt.Sprintf("/tmp/.mnt_%s", util.RandString(5))
...
func (p mountDeviceS) Desc() string {
	return "escape privileged container via disk mount, usage: `./cdk run device-boundary`"
}
