//go:build !no_docker_sock_boundary
// +build !no_docker_sock_boundary
...
func (p DINDAttackDeployS) Desc() string {
	return "Create and run <cmd> in a container with host root `/` mounted to container `/host` usage: ./cdk run docker-sock-deploy <sock_path> <shell_cmd> example: ./cdk run docker-sock-boundary /var/run/docker.sock \"touch /host/tmp/pwn-success\""
}
