//go:build !no_unpriv_userns_check && linux
// +build !no_unpriv_userns_check,linux
...
// - write payload code to /exp.sh in container
// - run `chmod +x /exp.sh`
...
func (exp ExploitUnprivUserNS) Desc() string {
	return "abuse cgroup-boundary co-operating with unprivileged user namespace creation. usage: ./cdk run unpriv-userns-boundary \"shell-cmd-payloads\""
}
