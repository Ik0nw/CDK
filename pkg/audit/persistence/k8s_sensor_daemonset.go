//go:build !no_sensor_daemonset
// +build !no_sensor_daemonset
...
func (p K8sBackDoorDaemonsetS) Desc() string {
	return "deploy image to every node using daemonset, usage: cdk run k8s-sensor-daemonset (default|anonymous|<service-account-token-path>) <image> <cmd>"
}
